...
/* get run mode */
#define EXPUSB_IOC_GETPOWER         _IOR(EXPUSB_IOC_MAGIC,  0x2E, struct expusb_val *)
...
static inline int  expusb_helper_setbklight(int fd, unsigned int port, unsigned int level)
{
    ...
    printf("bklight set port: %d,val: %d\r\n", port, level);
...
static inline int  expusb_helper_getnextpower(int fd, struct expusb_val * power)
{
    if (!power)
    {
        return -EFAULT;
    ...
    return ioctl(fd, EXPUSB_IOC_GETPOWER, (unsigned long)power);
}
