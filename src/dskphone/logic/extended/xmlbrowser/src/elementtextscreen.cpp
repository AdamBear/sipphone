...
#if ENABLETALKANDVOICE
#include "Talk/Talklogic_common.h"
#endif
CElementTextScreen::CElementTextScreen()
{
}

// 创建之后的处理
bool CElementTextScreen::AfterCreate()
{
    return CElementHaveUIBase::AfterCreate();
}

// 销毁之前的处理
void CElementTextScreen::BeforeDestroy()
{

}

// 是否播放提示音
bool CElementTextScreen::IsNeedPlayBeep()
{
    return _GetXData().stAttr.bBeep;
}

// softkey处理
bool CElementTextScreen::ProcessSoftkey(XB_SOFTKEY_TYPE eType, int iFocusedItem, bool & bUpdate)
{
    // 暂无处理
    bUpdate = false;
    return false;
...
// 对UI事件的处理
bool CElementTextScreen::OnAction(XB_ACTION_TYPE eAction, void * pData, bool & bExitWnd,
