...
#include <sys/time.h>
#include "kmreview/mod_kmreview.h"
// 控制触屏点信息调试信息宏开关
#define KEYBOARD_DEBUG_INFO

#ifdef KEYBOARD_DEBUG_INFO
...
void printTime(const char * lpszFmt, ...)
{
    ...
    fprintf(stdout, "Time:(%ld.%ld)\tKeyboard Plugin:\t",
            (tData.tv_sec % 1000) * 1000 + tData.tv_usec / 1000, tData.tv_usec % 1000);
    vfprintf(stdout, lpszFmt, argList);
    fprintf(stdout, "\n");
    va_end(argList);
}

class QYealinkKbPrivate : public QObject
{
    Q_OBJECT
public:
    QYealinkKbPrivate(QYealinkInputKeyboardHandler *, const QString &);
    ~QYealinkKbPrivate();
...
private:
    QYealinkInputKeyboardHandler * m_handler;
    int                           m_fd;
    int                           m_tty_fd;
...
QYealinkInputKeyboardHandler::QYealinkInputKeyboardHandler(const QString & device)
    : QWSKeyboardHandler(device)
{
    d = new QYealinkKbPrivate(this, device);
}

QYealinkInputKeyboardHandler::~QYealinkInputKeyboardHandler()
{
    delete d;
}

bool QYealinkInputKeyboardHandler::filterInputEvent(quint16 &, qint32 &)
{
    return false;
}

QYealinkKbPrivate::QYealinkKbPrivate(QYealinkInputKeyboardHandler * h, const QString & device)
    : m_handler(h), m_fd(-1), m_tty_fd(-1), m_orig_kbmode(K_XLATE)
{
    ...
    m_fd = open(dev.toLocal8Bit().constData(), O_RDWR, 0);
    if (m_fd >= 0)
    {
...
QYealinkKbPrivate::~QYealinkKbPrivate()
{
    if (m_tty_fd >= 0)
    {
...
void QYealinkKbPrivate::switchLed(int led, bool state)
{
    ...
    write(m_fd, &led_ie, sizeof(led_ie));
}

void QYealinkKbPrivate::readKeycode()
{
    ...
    forever
    {
        n = read(m_fd, reinterpret_cast<char *>(buffer) + n, sizeof(buffer) - n);
    ...
    for (int i = 0; i < n; ++i)
    {
        ...
        // 记录用户触屏抬起
        DEBUG_LOG("keycode is %d, bDown is %d", code, value);
        mod_KM_RecordKey(code, value);
