...
void CDialogCache::Uninit()
{
    ...
    for (; itBegin != m_MapCacheDlg.end(); ++itBegin)
    {
        ...
        {
            itBegin->second.pDlg->deleteLater();
        ...
        const yl::string & strDlgObjectName /* = ""*/)
{
    ...
    {
        // 根据窗口类型创建
        pTargetDlg = CreateDlg(strDlgClassName);
        if (pTargetDlg == NULL)
        {
            return NULL;
        }
        ...
        //!!!!!
        if (/*GetCacheType(strDlgClassName) != DCT_NoCache */true
                && !IsDlgCached(strDlgName))
        {
            // 加入缓存中时要标记窗口已被使用
...
void CDialogCache::PutPrivateDlg(CBaseDialog * pDlg)
{
    ...
    if (eWT_Idle == pDlg->GetWindowType())
    {
        UIMANAGER_ERR("CDialogCache::PutPrivateDlg put idlescreen!!!");
        return;
    }
    ...
    {
        UIMANAGER_INFO("CDialogCache::PutPrivateDlg no found (class:%s, name:%s)",
                       pDlg->metaObject()->className(), pDlg->objectName().toUtf8().data());
    ...
    UIMANAGER_DEBUG("CDialogCache::PutPrivateDlg(class:%s, name:%s)", pDlg->metaObject()->className(),
                    pDlg->objectName().toUtf8().data());
...
{
    // 设置ObjectName
    pDlg->setObjectName(strDlgObjectName.c_str());

    SDlgInfo tmpInfo = {pDlg, strDlgClassName, bUsed};
    m_MapCacheDlg.insert(pair<yl::string, SDlgInfo>(strDlgObjectName, tmpInfo));
}
...
{
    // 根据类名创建，类似于Java类反射
    CBaseDialog * pDlg = CDialogFactory::CreateClassByName(strDlgClassName.c_str());
    if (pDlg != NULL)
    {
        return pDlg;
    }

    chWARNING2(FALSE, commonAPI_FormatString("[UIManager] Unknown Dialog Class Name [%s]!",
               strDlgClassName.c_str()).c_str());
    return NULL;
}
