#ifndef SETTING_TEXTMESSAGEMODEL_H_
#define SETTING_TEXTMESSAGEMODEL_H_
...
{
// Class CTextMessageModel, 文本信息的Model.
...
public:
    ...
    typedef std::map<yl::string, int> UnReadMap;
    ...
    // | listIndex | 是要删除的下标列表.
    // 如果删除成功则返回true, 否则返回false.
    bool RemoveMessageList(int nType, const std::list<int> & listIndex);
...
private:
    ...
    // | strUserName | 是账号的用户名.
    // 如果获取成功则返回当前数量, 否则返回0.
    int MyGetAccountUnRead(const yl::string & strUserName) const;

    // 获取所有未读信息数.
    // 如果获取成功则返回所有未读信息数, 否则返回0.
    int MyGetTotalUnRead() const;
...
private:
    ...
    friend class CTextMessageController;
};

}  // namespace NS_SETTING

#endif  // SETTING_TEXTMESSAGEMODEL_H_
