...
bool CCallForwardController::GetPageMenuListByClickAction(
...
const yl::string CCallForwardController::GetTitleByClickAction(
        ...
        int iAccountID)
{
    ...
    case FWD_TYPE_ALWAYS:
        {
            pForwardData->m_isOn = configParser_GetCfgItemIntValue(kszAccountAlwaysFWDStatus,
                                   iAccountID) == 1 ? TRUE : FALSE;
            pForwardData->m_strTarget = configParser_GetCfgItemStringValue(kszAccountAlwaysFWDTarget,
                                        iAccountID);
            pForwardData->m_strOnCode = configParser_GetCfgItemStringValue(kszAccountAlwaysFWDOnCode,
                                        iAccountID);
            pForwardData->m_strOffCode = configParser_GetCfgItemStringValue(kszAccountAlwaysFWDOffCode,
                                         iAccountID);
        }
        break;
    case FWD_TYPE_BUSY:
        {
            pForwardData->m_isOn = configParser_GetCfgItemIntValue(kszAccountBusyFWDStatus,
                                   iAccountID) == 1 ? TRUE : FALSE;
            ...
            pForwardData->m_strOffCode = configParser_GetCfgItemStringValue(kszAccountBusyFWDOffCode,
                                         iAccountID);
        }
        break;
    case FWD_TYPE_NO_ANSWER:
        {
            pForwardData->m_isOn = configParser_GetCfgItemIntValue(kszAccountNoASWFWDStatus,
                                   iAccountID) == 1 ? TRUE : FALSE;
            pForwardData->m_strTarget = configParser_GetCfgItemStringValue(kszAccountNoASWFWDTarget,
                                        iAccountID);
            pForwardData->m_strOnCode = configParser_GetCfgItemStringValue(kszAccountNoASWFWDOnCode,
                                        iAccountID);
            pForwardData->m_strOffCode = configParser_GetCfgItemStringValue(kszAccountNoASWFWDOffCode,
                                         iAccountID);
...
bool CCallForwardController::SetPhoneForwardDataByType(FWD_TYPE eType, const ForwardData & refData)
{
    ...
    case FWD_TYPE_ALWAYS:
        {
            bChanged = configParser_SetConfigInt(kszForwardAlwaysSwitch, refData.m_isOn, CONFIG_LEVEL_PHONE)
                       || bChanged;
            bChanged = configParser_SetConfigString(kszForwardAlwaysTarget, refData.m_strTarget.c_str(),
                                                    CONFIG_LEVEL_PHONE) || bChanged;
            bChanged = configParser_SetConfigString(kszForwardAlwaysOnCode, refData.m_strOnCode.c_str(),
                                                    CONFIG_LEVEL_PHONE) || bChanged;
            bChanged = configParser_SetConfigString(kszForwardAlwaysOffCode, refData.m_strOffCode.c_str(),
                                                    CONFIG_LEVEL_PHONE) || bChanged;
        }
        break;
    case FWD_TYPE_BUSY:
        {
            bChanged = configParser_SetConfigInt(kszForwardBusySwitch, refData.m_isOn, CONFIG_LEVEL_PHONE)
                       || bChanged;
            bChanged = configParser_SetConfigString(kszForwardBusyTarget, refData.m_strTarget.c_str(),
                                                    CONFIG_LEVEL_PHONE) || bChanged;
            bChanged = configParser_SetConfigString(kszForwardBusyOnCode, refData.m_strOnCode.c_str(),
                                                    CONFIG_LEVEL_PHONE) || bChanged;
            bChanged = configParser_SetConfigString(kszForwardBusyOffCode, refData.m_strOffCode.c_str(),
                                                    CONFIG_LEVEL_PHONE) || bChanged;
        }
        break;
    case FWD_TYPE_NO_ANSWER:
        {
            bChanged = configParser_SetConfigInt(kszForwardNoAnswerSwitch, refData.m_isOn, CONFIG_LEVEL_PHONE)
                       || bChanged;
            bChanged = configParser_SetConfigString(kszForwardNoAnswerTarget, refData.m_strTarget.c_str(),
                                                    CONFIG_LEVEL_PHONE) || bChanged;
            bChanged = configParser_SetConfigInt(kszForwardNoAnswerDelayTime, refData.m_nRingTimes,
                                                 CONFIG_LEVEL_PHONE) || bChanged;
            bChanged = configParser_SetConfigString(kszForwardNoAnswerOnCode, refData.m_strOnCode.c_str(),
                                                    CONFIG_LEVEL_PHONE) || bChanged;
            bChanged = configParser_SetConfigString(kszForwardNoAnswerOffCode, refData.m_strOffCode.c_str(),
                                                    CONFIG_LEVEL_PHONE) || bChanged;
        }
        break;
    default:
        ...
        int iAccountID)
{
    ...
    case FWD_TYPE_ALWAYS:
        {
            bChanged = configParser_SetCfgItemIntValue(kszAccountAlwaysFWDStatus, iAccountID, refData.m_isOn,
                       CONFIG_LEVEL_PHONE) || bChanged;
            bChanged = configParser_SetCfgItemStringValue(kszAccountAlwaysFWDTarget, iAccountID,
                       refData.m_strTarget, CONFIG_LEVEL_PHONE) || bChanged;
            bChanged = configParser_SetCfgItemStringValue(kszAccountAlwaysFWDOnCode, iAccountID,
                       refData.m_strOnCode, CONFIG_LEVEL_PHONE) || bChanged;
            bChanged = configParser_SetCfgItemStringValue(kszAccountAlwaysFWDOffCode, iAccountID,
                       refData.m_strOffCode, CONFIG_LEVEL_PHONE) || bChanged;
        }
        break;
    case FWD_TYPE_BUSY:
        {
            bChanged = configParser_SetCfgItemIntValue(kszAccountBusyFWDStatus, iAccountID, refData.m_isOn,
                       CONFIG_LEVEL_PHONE) || bChanged;
            bChanged = configParser_SetCfgItemStringValue(kszAccountBusyFWDTarget, iAccountID,
                       refData.m_strTarget, CONFIG_LEVEL_PHONE) || bChanged;
            bChanged = configParser_SetCfgItemStringValue(kszAccountBusyFWDOnCode, iAccountID,
                       refData.m_strOnCode, CONFIG_LEVEL_PHONE) || bChanged;
            bChanged = configParser_SetCfgItemStringValue(kszAccountBusyFWDOffCode, iAccountID,
                       refData.m_strOffCode, CONFIG_LEVEL_PHONE) || bChanged;
        }
        break;
    case FWD_TYPE_NO_ANSWER:
        {
            bChanged = configParser_SetCfgItemIntValue(kszAccountNoASWFWDStatus, iAccountID, refData.m_isOn,
                       CONFIG_LEVEL_PHONE) || bChanged;
            bChanged = configParser_SetCfgItemStringValue(kszAccountNoASWFWDTarget, iAccountID,
                       refData.m_strTarget, CONFIG_LEVEL_PHONE) || bChanged;
            bChanged = configParser_SetCfgItemStringValue(kszAccountNoASWFWDOnCode, iAccountID,
                       refData.m_strOnCode, CONFIG_LEVEL_PHONE) || bChanged;
            bChanged = configParser_SetCfgItemStringValue(kszAccountNoASWFWDOffCode, iAccountID,
                       refData.m_strOffCode, CONFIG_LEVEL_PHONE) || bChanged;
            bChanged = configParser_SetCfgItemIntValue(kszAccountNoASWFWDTimeout, iAccountID,
                       refData.m_nRingTimes, CONFIG_LEVEL_PHONE) || bChanged;
        }
        break;
    default:
        {
