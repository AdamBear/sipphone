...
bool CNetworkController::GetPageMenuListByClickAction(
...
const yl::string CNetworkController::GetTitleByClickAction(
...
// SetWanIpMode
// iMode:0-IPv4,1-IPv6,2-IPv4 and IPv6;iIPreference:0-IPv4,1-IPv6;
bool CNetworkController::SetWanIPMode(int iMode, int iIPreference /*= 0*/)
{
    if (iMode == IPAM_BOTH)
    {
        return configParser_SetConfigInt(kszNetworkIPAddressMode, iMode, CONFIG_LEVEL_PHONE) && configParser_SetConfigInt(kszNetWorkPreference, iIPreference, CONFIG_LEVEL_PHONE);
    }
    else
    {
...
void CNetworkController::SetWanTypeData(const NetworkWanTypeData & refData)
{
    ...
    configParser_SetConfigInt(kszNetworkWanType, refData.m_eType);
}

void CNetworkController::SetIPv6WanTypeData(const NetworkWanTypeData & refData)
{
    // Set IPv6 network WAN type.
    configParser_SetConfigInt(kszNetworkIPv6WANType, refData.m_eType);
}
...
bool CNetworkController::SetWanStaticDnsServerData(const NetworkWanDnsServerData & refData)
{
    ...
    if (IPAM_IPV4 == refData.m_nMode)
    {
        ...
        bChanged = configParser_SetConfigString(kszNetworkWanStaticPriDns,
                                                refData.m_strWanPrimaryDns.c_str()) || bChanged;
        // Set secondary dns.
        bChanged = configParser_SetConfigString(kszNetworkWanStaticSecDns,
                                                refData.m_strWanSendaryDns.c_str()) || bChanged;
    }
    else
    {
        // Set static dns enable.
...
{
    // Set static ip.
    configParser_SetConfigString(kszNetworkWanStaticIp, refData.m_strWanIp.c_str());
    // Set static mask.
...
void CNetworkController::SetWanPppoeData(const NetworkWanPppoeData & refData)
{
    ...
    // Set pppoe pin.
    configParser_SetConfigString(kszNetworkPppoePin, refData.m_strPppoePin.c_str());
}
...
{
    // Set VLAN WAN status.
    configParser_SetConfigInt(kszVlanWanSwitch, refData.m_isWanEnable ? 1 : 0);
    // Set VLAN WAN VID.
    configParser_SetConfigInt(kszVlanWanVid, refData.m_nWanVid);
    // Set VLAN WAN priority.
    configParser_SetConfigInt(kszVlanWanPriority, refData.m_nWanPriority);

    // Set VLAN PC status.
    configParser_SetConfigInt(kszVlanPcSwitch, refData.m_isPcEnable ? 1 : 0);
    // Set VLAN PC VID.
    configParser_SetConfigInt(kszVlanPcVid, refData.m_nPcVid);
    // Set VLAN PC priority.
    configParser_SetConfigInt(kszVlanPcPriority, refData.m_nPcPriority);
}
...
bool CNetworkController::SetLLDPData(const NetworkLLDPData & refData)
{
    ...
    // Set lldp status.
    bChanged = configParser_SetConfigInt(kszLLDPSwitch, refData.m_bEnable ? 1 : 0) || bChanged;
    // Set lldp Packet Interval
    bChanged = configParser_SetConfigInt(kszLLDPPacketInterval, refData.m_nPacketInterval) || bChanged;
...
bool CNetworkController::SetVlanDHCPData(const NetworkVlanDHCPData & refData)
{
    ...
    bChanged = configParser_SetConfigInt(kszVlanDhcpSwitch, refData.m_bEnable ? 1 : 0) || bChanged;
    // Set VLAN DHCP option.
    bChanged = configParser_SetConfigString(kszVlanDhcpOption, refData.m_strOption.c_str()) || bChanged;
...
void CNetworkController::SetWebServerData(const NetworkWebServerData & refData)
{
    ...
    configParser_SetConfigString(kszHttpServerPort, refData.m_strHTTP_Port.c_str());
...
{
    // Set 802.1x mode.
    configParser_SetConfigInt(ksz802Dot1xMode, refData.m_eMode);
    // Set 802.1x id.
    configParser_SetConfigString(ksz802Dot1xId, refData.m_strId.c_str());
    // Set 802.1x password.
...
{
    // Set vpn status.
    configParser_SetConfigInt(kszVpnOnSwitch, refData.m_isEnable ? 1 : 0);
}
