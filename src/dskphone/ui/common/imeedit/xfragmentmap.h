...
// 移植Qt的 qfragmentmap_p.h

#include <stdlib.h>
#include "ETLLib/_chDefine.h"
...
#define X_CHECK_PTR(p) do { if (!(p)) xBadAlloc(); } while (0);
}
...
public:
    ...
    {
        chASSERT(field < Fragment::size_array_max);
    ...
    {
        chASSERT(!head->root || !fragment(head->root)->parent);
        return head->root;
    }
    inline void setRoot(uint new_root)
    {
        chASSERT(!head->root || !fragment(new_root)->parent);
        head->root = new_root;
    }
...
template <class Fragment>
xFragmentMapData<Fragment>::xFragmentMapData()
    : fragments(0)
{
    init();
}
...
template <class Fragment>
void xFragmentMapData<Fragment>::init()
{
...
template <class Fragment>
uint xFragmentMapData<Fragment>::createFragment()
{
    chASSERT(head->freelist <= head->allocated);

    uint freePos = head->freelist;
    if (freePos == head->allocated)
    {
        // need to create some free space
        uint needed = xAllocMore((freePos + 1) * fragmentSize, 0);
        chASSERT(needed / fragmentSize > head->allocated);
        Fragment *newFragments = (Fragment *)realloc(fragments, needed);
        X_CHECK_PTR(newFragments);
        fragments = newFragments;
...
template <class Fragment>
void xFragmentMapData<Fragment>::freeFragment(uint i)
{
...
template <class Fragment>
uint xFragmentMapData<Fragment>::next(uint n) const
{
    chASSERT(n);
    if (F(n).right)
    {
...
template <class Fragment>
uint xFragmentMapData<Fragment>::previous(uint n) const
{
    if (!n)
    {
        return maximum(root());
...
*/
template <class Fragment>
void xFragmentMapData<Fragment>::rotateLeft(uint x)
{
    uint p = F(x).parent;
    uint y = F(x).right;
...
*/
template <class Fragment>
void xFragmentMapData<Fragment>::rotateRight(uint x)
{
    uint y = F(x).left;
    uint p = F(x).parent;
...
template <class Fragment>
void xFragmentMapData<Fragment>::rebalance(uint x)
{
    F(x).color = Red;
...
template <class Fragment>
uint xFragmentMapData<Fragment>::erase_single(uint z)
{
    uint w = previous(z);
    uint y = z;
    uint x;
...
template <class Fragment>
uint xFragmentMapData<Fragment>::findNode(int k, uint field) const
{
    chASSERT(field < Fragment::size_array_max);
    uint x = root();
...
template <class Fragment>
uint xFragmentMapData<Fragment>::insert_single(int key, uint length)
{
    chASSERT(!findNode(key) || (int)this->position(findNode(key)) == key);
...
template <class Fragment>
int xFragmentMapData<Fragment>::length(uint field) const
{
    uint root = this->root();
...
template <class Fragment> // NOTE: must inherit xFragment
class xFragmentMap
{
public:
    class Iterator
    ...
    public:
        ...
        Iterator() : pt(0), n(0) {}
        Iterator(xFragmentMap *p, int node) : pt(p), n(node) {}
    ...
    public:
        ...
        ConstIterator(const xFragmentMap *p, int node) : pt(p), n(node) {}
...
private:
    ...
    xFragmentMapData<Fragment> data;

    xFragmentMap(const xFragmentMap& m);
    xFragmentMap& operator= (const xFragmentMap& m);
};


#endif // __FRAGMENT_MAP_H__
