 ...
 * key, value and optionally the map container type (defaults to YLHashMap)
 * By using the YLHashMap and a linked list of keys it allows O(1)
 ...
 *      Key - key type
 *      Value - value type
 *      MapType - an associative container like YLHashMap
 *      LockType - a lock type derived from the Lock class (default: NullLock = no synchronization)
 */
template<class Key, class Value,
         class MapType = YLHashMap<Key, Node<Key, Value>*>,
...
//  };
    ...
    {
        ScopedLock scoped(m_lock);
        printf("Cache Size : %d (max:%d) (elasticity: %d)\n"
               , (int)m_cache.size(), (int)m_maxSize, (int)m_elasticity);
