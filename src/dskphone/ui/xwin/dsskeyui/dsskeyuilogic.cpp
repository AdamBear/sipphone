    ...
    case CONFIG_MSG_BCAST_CHANGED:
        {
            ...
            else if (ST_SWITCH_PAGE == msg.wParam)
            {
                _DssKeyUILogic.UpdatePageMode();
                return TRUE;
            }
        }
        break;
...
CDssKeyUILogic::CDssKeyUILogic()
{
    ...
    m_bFullPageMode = configParser_GetConfigInt(kszSwitchPageKeyMode) != 0;
    m_iCurPageNum = 1;
    m_iPageTotal = 1;
    m_iUpdateCount = 0;
...
int CDssKeyUILogic::GetMaxNumPerPage() const
{
    return m_iMaxNumPerPage;
}
...
int CDssKeyUILogic::GetMaxNumber() const
{
    return m_iMaxNumber;
}
...
void CDssKeyUILogic::RefreshDssKeyBtn(int index, BOOL bForce/* = FALSE*/)
{
            ...
            && m_vecCurrentShow[index]->bNeedRefresh)
    {
        ...
        g_pDsskeySubject->NotifyByIndex(index, *m_vecCurrentShow[index]);
        g_pDssKeyLightLogic->UpdateLight(index,
                                         m_vecCurrentShow[index]->eLightColorType,
                                         m_vecCurrentShow[index]->eLightFlushType,
                                         m_vecCurrentShow[index]->iFlushuration);/**/
        m_vecCurrentShow[index]->bNeedRefresh = FALSE;
    }
}

void CDssKeyUILogic::RefreshMultipleDsskeyData(int iBegin, int num)
{
    ListLogicData DsskeyDataList;
    dsskey_GetData(DMT_LINEKEY_MODULE, iBegin, num, DsskeyDataList);
    ListLogicData::const_iterator it = DsskeyDataList.begin();
    for (/* */; it != DsskeyDataList.end(); ++it)
    {
        DssKeyLogicData DkLogicData = (DssKeyLogicData)(*it);
        UpdateLineKeyStatus(DkLogicData);
    }
}
...
bool CDssKeyUILogic::UpdateLineKeyStatus(DssKeyLogicData & objData)
{
    ...
    // dsskey对应界面上的button
    int iButtonIndex = iLKIdx % m_iLinenumPerPage;
...
//  }
    ...
    // 更新Dsskey显示信息
    bool bRet = dsskeyUI_ParseLogicData(m_vecDsskeyInfos[iLKIdx], objData);
    if (!bRet)
    {
...
#if IF_BUG_32429
    if (objData.eLightType != LT_NONE)
    {
        m_vecDsskeyInfos[iLKIdx].eLightColorType = GetColorType(objData.eLightType);
        m_vecDsskeyInfos[iLKIdx].eLightFlushType = GetFlushType(objData.eLightType);
    }
#endif
...
//#endif
    ...
    if (bHaveLockKey || m_bFullPageMode)
    {
        // 如果有按键被锁住，则会变成3页
...
int MapFlushTime(DSS_KEY_FLUSH_TYPE eFlushType)
{
    switch (eFlushType)
    {
...
void CDssKeyUILogic::RefreshPageKey()
{
    ...
    DSSKEYUI_INFO("m_iPageTotal(%d), m_iCurPageNum(%d), m_iCurrentPageIndex(%d).", m_iPageTotal,
                  m_iCurPageNum, m_iCurrentPageIndex);
    if (m_vecCurrentShow[m_iMaxNumPerPage - 1]->bNeedRefresh == TRUE)
    {
        // 有变化，先关灯，后根据当前Dsskey状态更新灯状态
        // 设置Page键对应的灯状态
        g_pDssKeyLightLogic->UpdateLight(m_iPageIndex, LED_COLOR_OFF, REFRESH_TYPE_NO_FLUSH);
    }
    // PageButton可用时
            ...
            && m_bEnablePageTips)
    {
            ...
            for (int iKeyIndex = 0; iKeyIndex < m_iLinenumPerPage; ++iKeyIndex)
            {
                ...
                if (m_vecDsskeyInfos[iCurrentKeyIndex].ePageColorType != LED_COLOR_OFF)
                {
                    if (iHighest == -1
                            ...
                            || (m_vecDsskeyInfos[iCurrentKeyIndex].iLightPriority == m_vecDsskeyInfos[iHighest].iLightPriority
                                && m_vecDsskeyInfos[iCurrentKeyIndex].iTimestamp >
                                m_vecDsskeyInfos[iHighest].iTimestamp))// 相同优先级后来优先
                    {
            ...
            if (i != m_iCurrentPageIndex)
            {
                ...
                else
                {
                    g_pDsskeySubject->OnNotifyPageIcon(i + 1, m_vecPageButtonInfo[i].ePageColorType, \
                                                       m_vecPageButtonInfo[i].ePageLightFlushType);
                }

                if (iHighest == -1
                        || m_vecPageButtonInfo[i].iLightPriority < m_vecPageButtonInfo[iHighest].iLightPriority
                        || (m_vecPageButtonInfo[i].iLightPriority == m_vecPageButtonInfo[iHighest].iLightPriority
                            && m_vecPageButtonInfo[i].iTimestamp >
                            m_vecPageButtonInfo[iHighest].iTimestamp))// 优先级相同后来的优先
                {
                    iHighest = i;
                }
            }
        ...
        if (iHighest >= 0)
        {
            g_pDssKeyLightLogic->UpdateLight(m_iPageIndex, \
                                             m_vecPageButtonInfo[iHighest].ePageColorType, m_vecPageButtonInfo[iHighest].ePageLightFlushType);
        }
        else
        {
            // 设置Page键对应的灯状态
                                       ...
                                       DSS_KEY_FLUSH_TYPE eFlushType)
{
    ...
    m_vecDsskeyInfos[iIndex].eLightFlushType = eFlushType;
    if (LED_COLOR_OFF == eColorType)
    {
        m_vecDsskeyInfos[iIndex].iFlushuration = 0;
    }
    // T46亮灯逻辑需重新整理
    if (m_iCurPageNum > 1)
    {
                ...
                && iIndex < (m_iCurrentPageIndex + 1) * m_iLinenumPerPage)
        {
            ...
            g_pDssKeyLightLogic->UpdateLight(iIndex, eColorType, eFlushType);
        }
    }
    else
    {
        if (iIndex < m_iMaxNumPerPage)
        {
            g_pDssKeyLightLogic->TurnOffLight(iIndex);
            g_pDssKeyLightLogic->UpdateLight(iIndex, eColorType, eFlushType);
        }
...
DSS_KEY_COLOR_TYPE CDssKeyUILogic::GetColorType(const LIGHT_Type eLightType)
{
    DSS_KEY_COLOR_TYPE eColorType = LED_COLOR_OFF;
    switch (eLightType)
    {
    case LT_GREEN_ON:
    ...
    case LT_GREEN_FLASH_4:
        {
            eColorType = LED_COLOR_TYPE_GREEN;
        }
        break;
    case LT_RED_ON:
    case LT_RED_FLASH_1:
    ...
    case LT_RED_GREEN_FLASH_4:
        {
            eColorType = LED_COLOR_RED_GREEN;
        }
        break;
    default:
        break;
...
DSS_KEY_FLUSH_TYPE CDssKeyUILogic::GetFlushType(const LIGHT_Type eLightType)
{
    DSS_KEY_FLUSH_TYPE eFlushType = REFRESH_TYPE_NO_FLUSH;
    switch (eLightType)
    {
    case LT_RED_FLASH_1:
    case LT_GREEN_FLASH_1:
    case LT_RED_GREEN_FLASH_1:
        {
            eFlushType = REFRESH_TYPE_FASTER_FLUSH;
        }
        break;
    case LT_RED_FLASH_2:
    case LT_GREEN_FLASH_2:
    case LT_RED_GREEN_FLASH_2:
        {
            eFlushType = REFRESH_TYPE_FAST_FLUSH;
        }
        break;
    case LT_RED_FLASH_3:
    case LT_GREEN_FLASH_3:
    ...
    case LT_RED_GREEN_FLASH_4:
        {
            eFlushType = REFRESH_TYPE_SLOW_FLUSH;
        }
        break;
    default:
        break;
...
void CDssKeyUILogic::UpdatePageMode()
{
    m_bFullPageMode = configParser_GetConfigInt(kszSwitchPageKeyMode) != 0;
