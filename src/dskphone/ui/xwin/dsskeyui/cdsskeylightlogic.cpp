...
void CDssKeyLightLogic::UpdateFlashTimer(int iDsskeyID, int iDelay, BOOL bFlash)//delay就是flushtime
{
	...
	{
		// 获取旧定时器类型
		if (iDelay == m_arryFlashTimer[i].iDelay)//M_arrayFlashTimer[i]这个定时器和我们需要使用的定时器类型是一样的
		{
			iNewTimerIndex = i;//因此我们可以直接使用，将新定时器的index直接设置为这个
		}
		// 获取新定时器类型
		if (pLightInfo != NULL
			&& m_arryFlashTimer[i].iDelay == pLightInfo->iDelay)//这边的是这个dsskey原来就有设置过 m_arryFlashTimer[i]这个定时器的，
		{
...
void CDssKeyLightLogic::updateLightStatus(int iDsskeyID, DSS_KEY_COLOR_TYPE eColorType, DSS_KEY_FLUSH_TYPE eFlushType)
{
	...
	switch (eFlushType)
	{
	case REFRESH_TYPE_SLOW_FLUSH://慢闪
		iFlashTime = DSSK_SLOWER_FLASH_TIMER;//1000ms
		break;

	case REFRESH_TYPE_FLUSH://闪动500ms
		iFlashTime = DSSK_SLOW_FLASH_TIMER;
		break;

	case REFRESH_TYPE_FAST_FLUSH://快闪 300ms
		iFlashTime = DSSK_FAST_FLASH_TIMER;
		break;

	case REFRESH_TYPE_FASTER_FLUSH://快快闪 200ms
