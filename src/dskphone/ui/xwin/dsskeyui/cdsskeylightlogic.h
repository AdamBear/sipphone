...
struct LIGHT_INFO 
{
	...
	int					iLightID;
	DSS_KEY_COLOR_TYPE	eLightType; // 灯颜色,红绿交替闪时指示当前灯状态,取值红色和绿色
	int					iDelay;
...
private:
	LightStatusMap		m_lightStatusMap;				//记录DssKey灯的状态，已DssKeyID为标识
	UINT				m_lightStatus;	//记录DssKey灯的状态，10个灯分别用10个bit标识，DssKeyLight1:1<<1,DssKeyLight2:1<<2,...
