...
int https_recv_head_2(FILE_INFO_ST *pcSerUrl, PROXY_ST stProxy, TimeOut_st stTimeOUt, 
					  unsigned int unMaxFileSize,char caFtpHead[HTTP_HEAD_SIZE], 
					  int *pnHeadSize, SSL** ssl, SSL_CTX **ctx,  int *pnFdData, 
					  int *pnFileSize, Err_st *pstErr);

int https_recv_to_file_2(FILE_INFO_ST *pcSerUrl, PROXY_ST stProxy, TimeOut_st stTimeOUt, 
						 unsigned int unMaxFileSize, char *pcLocalName, Err_st *pstErr);

int https_recv_to_ram_2(FILE_INFO_ST *pcSerUrl, PROXY_ST stProxy, TimeOut_st stTimeOUt, 
						unsigned int unMaxFileSize,char **ppcRam, int *pnFileSize, Err_st *pstErr);
...
*输入参数：num : 希望获取的证书的总个数。

*输出参数：pstCaDirInfo: 证书的个数和各证书的相关信息

*返回结果：    -1: 失败
nCaNum: CA目录下获取到的证书的个数。

*/
int ca_dir_info_ex(int num, ca_info_st **pstCaInof);
