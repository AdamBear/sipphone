...
#define KPIOC_TEST_KEYSEN            _IOR('X', 0xB1, struct key_sense*)
...
static inline int keypad_helper_get_key_sense(int fd,struct key_sense *sen)
{
    if(fd>0 && sen)
    {
        if(ioctl(fd,KPIOC_TEST_KEYSEN,(unsigned long)sen)>=0)
        {
            return 0;
        }
    }
